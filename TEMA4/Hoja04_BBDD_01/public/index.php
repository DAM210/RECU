...
use function Hoja04BBDD01\Clases\{getEquipos as obtenerEquipos, getJugadores as obtenerJugadores, getPosiciones as getPosiciones, realizarTraspaso as realizarTraspaso};
...
<body>
    ...
    ?>
        <table class=tabla>
            ...
            <?php
            // Obtiene la lista de jugadores del equipo seleccionado usando la función getJugadores() de FuncionesBD
            foreach (obtenerJugadores($nombre) as $jugadores) {
                echo "<tr>";
                echo "<td>" . $jugadores[0] . "</td>";
                echo "<td>" . $jugadores[1] . " kg</td>";
                echo "</tr>";
            }
            ?>
        </table>
    ...
    <?php
        ...
        if (isset($_POST["enviar"])) {
            ...
            // Realiza el traspaso utilizando la función realizarTraspaso
            $traspasoExitoso = realizarTraspaso($equipo, $jugadorSeleccionado, $nombre, $procedencia, $altura, $peso, $posicion);

            // Verifica si el traspaso fue exitoso
            if ($traspasoExitoso) {
                echo "<p>Traspaso realizado exitosamente.</p>";
            } else {
                echo "<p>Error al realizar el traspaso.</p>";
            }
        }
    }
